...
async function getInputSelector(page) {
    ...
    for (const selector of inputSelectors) {
		...
		console.log('Input selector is not exist')
		}
    }
...
// Function to get the correct output selector
async function getOutputSelector(page) {
    ...
    for (const selector of outputSelectors) {
        const exists = (await page.$(selector)) !== null
        if (exists) {
          console.log('Output selector found')
            return selector // Return the selector if found
        } else {
		console.log('Output selector is not exist')
		}
    }

    console.error('Error: Unable to find a valid output selector.')
    return null // Return null if no valid selector is found
}
...
async function getButtonSelector(page) {
    ...
    for (const selector of buttonSelectors) {
        ...
        if (exists) {
            return selector // Return the selector if found
			console.log('Rephrasing button is found.')
        }
    }
...
async function submitForm(page) {
  ...
  console.log('Pressed Ctrl + Enter to Rephrase');
  await page.keyboard.up('Control');
...
async function selectParaphraser(page){
	const ButtonXPath="//h6[contains(., 'Paraphraser')]"
	    try {
        ...
        if (menuButton) {
            ...
            throw new Error('Failed to find button for Paraphraser')
        }
		        // Wait a moment for animation if needed
        await page.waitForTimeout(500) // Adjust timing as necessary
    } catch (error) {
        console.error(`Error when getting Paraphraser button`)
    }
}
...
async function selectLanguage(page, languageName) {
    ...
    try {
        ...
        const languageOptionXPath = `//p[contains(text(), "${languageName}")]`
    ...
    const result = await page.evaluate(
        (document, sanitizedValue) => {
            ...
            const slider = document.querySelector(
                'input[type="range"][area-label="Synonyms"]'
            )
            if (slider) {
...
async function quillbot(text, options = {}) {
    ...
    try {
        ...
        await page.goto('https://quillbot.com/paraphrasing-tool', { waitUntil: 'networkidle0' })

		// Select Paraphraser
		//await selectParaphraser(page)
        ...
        // Wait for input
		const outputSelector = await getOutputSelector(page)
        ...
        for (let i = 0; i < parts.length; i += 1) {
			...
			//console.log('buttonSelector is Ok')
			await page.waitForTimeout(5000)
            const isSubmitted = await submitForm(page)
            if (!isSubmitted) {
                // Handle submission failure
                console.log('Form submission failed. Exiting script.')
                return
            }
			console.log('submitForm is Ok')
